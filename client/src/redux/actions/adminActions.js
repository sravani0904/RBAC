...
export const getAllParent = () => async (dispatch) => {
  try {
    ...
    dispatch({ type: GET_ALL_PARENT, payload: data });
  } catch (error) {
    console.log("Redux Error", error);
  }
};
...
export const deleteParent = (formData) => async (dispatch) => {
  try {
    const { data } = await api.deleteParent(formData);
    alert("Parent Deleted");
    dispatch({ type: DELETE_PARENT, payload: true });
  } catch (error) {
    dispatch({ type: SET_ERRORS, payload: error.response.data });
  }
};
