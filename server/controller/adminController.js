...
export const adminSignup = async (req, res) => {
  try {
    ...
    const username = name || email || `ADM${Date.now()}`;
    const hashed = await bcrypt.hash(password || "123", 10);
    const admin = await Admin.create({ name, email, password: hashed, username, department: "GENERAL", contactNumber: 0, dob: "1970-01-01", passwordUpdated: true });
    const token = jwt.sign({ email: admin.email, id: admin._id, role: "admin" }, "sEcReT", { expiresIn: "1h" });
...
export const deleteParent = async (req, res) => {
  try {
    ...
    for (var i = 0; i < parents.length; i++) {
      ...
      await Parent.findOneAndDelete({ _id: parent });
    }
...
export const getStudentByQuery = async (req, res) => {
  try {
    ...
    if (!query) return res.status(400).json({ error: "No search query given" });
    const q = query.trim();
