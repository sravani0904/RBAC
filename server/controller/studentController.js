...
export const studentSignup = async (req, res) => {
  try {
    ...
    const username = name || email || `STU${Date.now()}`;
    const hashed = await bcrypt.hash(password || "123", 10);
    const student = await Student.create({ name, email, password: hashed, username, year: 1, department: "GENERAL", section: "A", dob: "1970-01-01", passwordUpdated: true });
    const token = jwt.sign({ email: student.email, id: student._id, role: "student" }, "sEcReT", { expiresIn: "1h" });
